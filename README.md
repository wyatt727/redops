...
```
   ANDROID HOST (APK)              NETHUNTER CHROOT (Kali)
  ┌──────────────────────┐       ┌──────────────────────┐
  │ Kotlin / Compose UI  │       │ /root/pentest/       │
  │ ├─ Target Dashboard  │       │ ├─ scripts/pq (CLI)  │
  │ ├─ Traffic Capture   │  su   │ ├─ spawn_gate.py     │
  │ ├─ IPC Monitor       │──────>│ ├─ pq_lib/ (modules) │
  │ ├─ HTTP Repeater     │chroot │ ├─ docs/ (guides)    │
  │ ├─ PQ Manager        │       │ ├─ targets/ (data)   │
  │ └─ AI Agent          │       │ └─ Claude CLI        │
  │                      │       │                      │
  │ Room DB, Hilt DI     │       │ Frida, jadx, Python  │
  └──────────────────────┘       └──────────────────────┘
             │                              │
             └── PqCommandExecutor ─────────┘
                 su → chroot → android-entry
```
